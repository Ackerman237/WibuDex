...
const POLICIES = {
  ...
  doujin: { mode: 'always', strict: true },
};
...
// ============================================================
// Auto-toggle idle: VPN dimatikan otomatis saat sudah tidak ada
// target yang bergantung padanya — mis. user pindah dari neko ke
// doujin. Koneksi ulang tetap otomatis via ensureConnectedRoute.
// ============================================================
let idleTimeoutMs = Number(process.env.VPN_IDLE_TIMEOUT_MS) || 3 * 60_000;

// Aktivitas terakhir per target (di-update oleh ensureVpn & report*)
const lastUsedAt = {};
...
// Target "butuh VPN" jika mode-nya always, atau sedang blocked (sticky).
function targetNeedsVpn(target) {
  const policy = getPolicy(target);
  return policy.mode === 'always' || Boolean(getTargetState(target).blocked);
}

function markTargetUsed(target) {
  lastUsedAt[target] = Date.now();
}

/**
 * Keputusan idle-teardown: matikan VPN hanya jika SEMUA benar:
 *  - ada provider aktif
 *  - TIDAK ada target yang sedang blocked (dia sedang bergantung pada VPN;
 *    pelepasannya biar probeDirect di request berikutnya yang memutuskan)
 *  - TIDAK ada target mode-'always' yang aktif dalam window idle terakhir
 * Recency aktivitas menggantikan counter in-flight: setiap request selalu
 * menyentuh ensureVpn/report* sehingga window selalu segar selama request
 * berjalan (request terpanjang pun < window idle).
 */
function idleTeardownDueNow(now = Date.now()) {
  if (!activeProvider) return false;

  const hasBlockedDependent = Object.keys(POLICIES).some(
    (t) => getTargetState(t).blocked
  );
  if (hasBlockedDependent) return false;

  const anyRecentlyUsed = Object.keys(POLICIES).some((t) => {
    if (!targetNeedsVpn(t)) return false;
    return now - (lastUsedAt[t] || 0) < idleTimeoutMs;
  });
  return !anyRecentlyUsed;
}
...
async function verifyProvider(provider, target) {
  ...
  expectedIp = currentIp;
  if (target === 'neko' || target === 'doujin') {
    try {
      const res = await fetchWithTimeout(TARGET_PROBE_URLS[target], VERIFY_TIMEOUT_MS);
...
export function getVpnStatus() {
  ...
  // Sisa waktu sampai VPN dimatikan otomatis (null = tidak berlaku:
  // VPN mati, atau ada target blocked yang sedang bergantung padanya)
  let idleDisconnectInMs = null;
  if (
    activeProvider &&
    !Object.keys(POLICIES).some((t) => getTargetState(t).blocked)
  ) {
    const lastAlwaysUse = Math.max(
      ...Object.keys(POLICIES)
        .filter((t) => getPolicy(t).mode === 'always')
        .map((t) => lastUsedAt[t] || 0)
    );
    if (Number.isFinite(lastAlwaysUse) && lastAlwaysUse > 0) {
      idleDisconnectInMs = Math.max(0, idleTimeoutMs - (Date.now() - lastAlwaysUse));
    }
  }
...
async function monitorTick() {
  ...
  // --- Auto-toggle idle: berlaku untuk SEMUA tipe provider ---
  if (idleTeardownDueNow()) {
    const from = activeProvider.name;
    await deactivateActive();
    emit('vpn:idle-disconnected', { provider: from });
    pushHistory('idle_disconnected', { provider: from });
    persist();
    stopMonitor();
    return;
  }

  // --- Watch perubahan IP: hanya bermakna untuk provider system VPN ---
  if (activeProvider.type !== 'system') return;
  const currentIp = await fetchPublicIp();
  if (!currentIp) return;
...
onVpnEvent('vpn:idle-disconnected', (d) =>
  logger.info(d, 'VPN dimatikan otomatis — tidak ada target yang membutuhkan')
);
...
export const _internals = {
  reset() {
    ...
    expectedIp = null;
    connectingPromise = null;
    for (const key of Object.keys(lastUsedAt)) delete lastUsedAt[key];
  },
  getTargetState,
  getHealth,
  setBaselineIp(ip) {
  ...
  // --- Hook auto-toggle idle (untuk test & operasional) ---
  setIdleTimeoutMs(ms) {
    if (Number.isFinite(ms) && ms > 0) idleTimeoutMs = ms;
  },
  getIdleTimeoutMs() {
    return idleTimeoutMs;
  },
  getLastUsedAt(target) {
    return lastUsedAt[target] || null;
  },
  setLastUsedAt(target, ts) {
    if (Number.isFinite(ts)) lastUsedAt[target] = ts;
  },
  setPolicyMode(target, mode) {
    if (POLICIES[target]) POLICIES[target].mode = mode;
  },
  // Jalankan evaluasi idle-teardown sekarang juga (monitor interval = 60 dtk,
  // hook ini membuat test & debugging tidak perlu menunggu tick berikutnya).
  async forceIdleCheck() {
    await monitorTick();
  },
};
