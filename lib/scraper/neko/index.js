...
import { parsePlayers, parseEpisodes, parseRelated, parseVideoMeta, collectPageThumbs } from './parsers/detail.js';
...
export async function scrapeNekoList(page = 1) {
  ...
  return { videos: parseCards(html), hasNext: parseHasNext(html, page) };
}
...
export async function scrapeNekoDetail(slug) {
  ...
  const html = await getHtml(`/${cleanSlug}/`);
  ...
  // Enrich thumbnail: episode/related sering tidak punya thumb sendiri —
  // ambil dari peta slug→thumb yang tersebar di seluruh halaman
  const pageThumbs = collectPageThumbs(html);
  const related = parseRelated(html).map((r) => ({
    ...r,
    thumb: r.thumb || pageThumbs[r.slug] || '',
  }));
  const episodes = parseEpisodes(html, cleanSlug).map((e) => ({
    ...e,
    thumb: e.thumb || pageThumbs[e.slug] || '',
  }));
  ...
  const detail = {
    ...
    synopsis,
    // Meta dari <meta name="description"> upstream (Genre/Duration/Producers)
    ...parseVideoMeta(html),
    related,
    episodes,
  };
...
export async function scrapeNekoSchedule() {
  ...
  const html = await getHtml('/jadwal-new-hentai/');
