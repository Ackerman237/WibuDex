...
import { curlGetText } from './player-frame.js';
 ...
 * Default cepat: timeout 12 dtk & TANPA tingkat Puppeteer — jalur stream harus
 * responsif; kalau fetch+curl gagal, caller jatuh ke player-frame terfilter.
 */
export async function extractDirectStream(embedUrl, { slug = '', timeoutMs = 12000, allowPuppeteer = false } = {}) {
  const { fetchProviderEmbed } = await import('./player-frame.js');
  const html = await fetchProviderEmbed(embedUrl, { slug, timeoutMs, allowPuppeteer });
