...
// Cache scrape: list/search 60 dtk (chapter baru cepat terlihat),
// detail & genres 5 menit (jarang berubah) — via ttl per-call.
const cache = new CacheManager({ defaultTTL: 60_000, maxSize: 200 });
const DETAIL_TTL_MS = Number(process.env.SCRAPE_DETAIL_TTL_MS) || 5 * 60_000;
...
async function apiGet(path, { includeMeta = false, ttl } = {}) {
  const cacheKey = path;
...
export async function scrapeMangaDetail(slug) {
  ...
  return mapDetail(await apiGet(`/manga/${encodeURIComponent(slug)}`, { ttl: DETAIL_TTL_MS }));
}

export async function scrapeGenres() {
  const response = await apiGet('/genres', { includeMeta: false, ttl: DETAIL_TTL_MS });
